    ...
    for nte=1:3650
        ...
        for i=2:n
            for j=1:layer(i)
                ...
                otpt(j,i)=1/(1+exp(-(inpt(j,i)))); % sigmoid (1/1+e^(-x)) evaluation f(x)
            end
%             if(f==1)
%                 break;
%             end
...
%         end
        ...
        for j=1:layer(n-1)
            sum(j,n-1)=0;
            for k=1:layer(n)
                delta2=err(k)*(1/(1+exp(-(inpt(k,n)))))*(1-(1/(1+exp(-(inpt(k,n)))))); %update with derrivative of f(x) which is f'(x)
        ...
        for i=1:layer(n-2)
            sum(i,n-2)=0;
            for j=1:layer(n-1)
                delta3=sum(j,n-1)*(1/(1+exp(-(inpt(j,n-1)))))*(1-(1/(1+exp(-(inpt(j,n-1))))));%update with derrivative of f(x) which is f'(x)
...
%                         fprintf('****>>>>>>>> %.20f\n',inpt(j,i));
%                     end
                    otpt(j,i)=1/(1+exp(-(inpt(j,i)))) ;           %sigmoid
                end
            end
