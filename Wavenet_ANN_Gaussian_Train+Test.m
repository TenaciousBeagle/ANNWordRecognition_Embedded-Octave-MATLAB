    ...
    for nte=1:3650
        ...
        for i=2:n
            for j=1:layer(i)
                ...
                otpt(j,i)=(inpt(j,i)/sf)*exp(-((inpt(j,i)/sf)^2)/2)/val; % gaussian wavelet evaluation f(x)
            end
%             if(f==1)
%                 break;
%             end
...
%         end
        ...
        for j=1:layer(n-1)
            sum(j,n-1)=0;
            for k=1:layer(n)
                delta2=err(k)*exp(-((inpt(k,n)/sf)^2)/2)*(1-(inpt(k,n)/sf)^2)/(sf*val); %update with derrivative of f(x) which is f'(x)
...
%                         fprintf('****>>>>>>>> %.20f\n',inpt(j,i));
%                     end
                    otpt(j,i)=(inpt(j,i)/sf)*exp(-((inpt(j,i)/sf)^2)/2)/val;            %gaussian wavelet
                end
            end
